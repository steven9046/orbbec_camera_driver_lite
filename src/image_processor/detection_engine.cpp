...
/** 1. 模型路径
 *  2. tensor info
 *  3. 创建   InferenceHelper
 *  4. 初始化 InferenceHelper
 */
/*** Function ***/
int32_t DetectionEngine::Initialize(const std::string& work_dir, const int32_t num_threads)
{
    ...
    input_tensor_info.normalize.mean[0] = 0.0f;     /* 0.0 - 1.0*/ // 这就相当用没有做
...
/**
 *  @param original_mat     传入的图片
 *  @param result           输出的结果
 *  1. 传入图片进行处理，做成一个tensor
 * 
 */
int32_t DetectionEngine::Process(const cv::Mat& original_mat, Result& result)
{
    ...
    std::cout << "original mat height: " << original_mat.rows << std::endl;
    std::cout << "original mat width : " << original_mat.cols << std::endl;
    /*** PreProcess ***/
    ...
    std::cout << "resized mat height: " << img_src.rows << std::endl;
    std::cout << "resized mat width : " << img_src.cols << std::endl;
    ...
    std::cout << "After Nms bbox size: " << bbox_nms_list.size() << std::endl;
    std::cout << "-----------------------------------------------------" << std::endl;
