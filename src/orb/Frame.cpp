...
Frame::Frame(): mpPrevFrame(NULL), mbIsSet(false)
{
...
/**
 * @brief 提取ORB特征
 * @param [in] flag 是否提取双目目图像(应该是双目的重叠部分？)
 * @param [in] im   图像
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    // if(flag==0)
    monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors);
    // std::cout << "disss: " << mDescriptors.size() << std::endl;
    // else
...
void Frame::UndistortKeyPoints()
{
    ...
    // Undistort points
    mat=mat.reshape(2); // 这里为什么要改成2XN？？
    cv::undistortPoints(mat,mat, mK, mDistCoef,cv::Mat(),mK);
    mat=mat.reshape(1); // 这里又改回一行??


    // Fill undistorted keypoint vector
    mvKeysUn.resize(N);// Nx2
    for(int i=0; i<N; i++)
    {
...
/**
 * @brief 使用针孔相机模型去畸变，图像的有效边界可能会改变
 */
...
/**
 * @brief 这里有问题
 * 我们的这个depth不是和RGB一一对应的，分辨率都不一样，只能从Depth里得到3D点，投影到RGB上，找到RGB和depth的对应关系
 * 1. Depth每一个点的depth相机系里的3D坐标
 * 2. 转到RGB相机坐标系里，通过相机模型计算出相应的像素坐标
 * 3. 根据对应关系得到特征点的 对应的 mvDepth
 */
